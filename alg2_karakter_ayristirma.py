...
for i,c in enumerate(cnt):
    ...
    kon1 = max([w,h])<W/4 #genişliğimiz/yüksekliğimiz ana resminin genişliğinin 4 te birinden küçük olmalı
    kon2 = w*h > 200 #Alan kontrolu
    
    if(kon1 and kon2): #karakterleri yakaladık
        ...
        minx = max(0,minx-odak) #eğer eksi bir değer alırsa sıfır olacak, kesme işlemi pozitif ekran noktalarından olur
        miny = max(0,miny-odak)
        maxx = min(W,maxx+odak) #W koyduk çünkü en fazla yüksekliğin genişliği olsun, yukarıdaki açıklama ile aynı sebep
